...
alter table agencies add constraint phone_number check ( phone_number ilike ('+996%'));
...
insert into agencies (name,phone_number, address_id) values ('House_kg','+996100200200', 1);
...
select house_type, o.first_name, max(length(o.first_name)) from owners o inner join houses h on o.id = h.owner_id group by house_type, o.first_name limit 1;

-- - 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select  h.furniture from houses h where price between '1500' and '2000';

-- - 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select a.id, house_type from houses h inner join addresses a on a.id = h.address_id where address_id in ('5', '6', '7', '8', '9', '10');

-- - 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select h.house_type, o.first_name, c.first_name, a.name  from houses h inner join rent_info r on h.id = r.house_id inner join owners o on r.id = o.id inner join customers c on r.customer_id = c.id inner join agencies a on r.agency_id = a.id;
...
select h.house_type, h.rating, o.first_name from houses h inner join rent_info ri on h.id = ri.house_id inner join owners o on o.id = h.owner_id order by rating asc;
